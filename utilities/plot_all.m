...
function plot_all(data,labels,timesteps, num_exp)
global net name finger fgr_idx which_finger another experiment type_prob label_num testing fm

num=num_exp;
if isempty(timesteps{num_exp})
    t=1:size(data{num_exp},2);
else
    t = timesteps{num_exp};
end
if testing==0
...
d_num = data{num_exp};
d = d_num(1:15,:);
l_pred_exp = classify(net,data{num_exp,1});
l = labels{num_exp};
% size(l)
...
if another==2
    title({title1,'Free movement experiment',title3 },'FontSize', fontSize);
else 
